...
function col_state_compare(col1, col2) {
    const col1_leq_col2 = col_state_leq(col1, col2);
    const col2_leq_col1 = col_state_leq(col2, col1);
...
function game_state_leq(game_state1, game_state2) {
    ...
    return generate_permutations(n).some(pi => {
        for (let i = 0; i < n; i++) {
            if (!col_state_leq(game_state1[i], game_state2[pi[i]])) {
                return false;
            }
        }
        return true;
...
function game_state_compare(game_state1, game_state2) {
    const gs1_leq_gs2 = game_state_leq(game_state1, game_state2);
    const gs2_leq_gs1 = game_state_leq(game_state2, game_state1);
